...
type Video = { src: string; name: string; title: string; poster: string; linkedin?: string; result?: string };
type Written = { quote: string; name: string; title: string; photo?: string; linkedin?: string; result?: string };
...
function Byline({ person }: { person: Written }) {
  return (
    <div className="flex items-center gap-3">
      {person.photo && (
        <img
          src={person.photo}
          alt={person.name}
          className="h-11 w-11 border-2 border-foreground object-cover"
        />
      )}
      <div>
  ...
  return (
    <section id="testimonials" className="bg-background py-20 md:py-28">
      <div className="mx-auto max-w-[1180px] px-6">
        ...
        {/* Video testimonials lead the section */}
        <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 md:mt-16">
          {videos.map((v) => (
            <button
              key={v.src}
              onClick={() => setActive(v)}
              className="group appearance-none p-0 text-left"
            >
              <div className="relative aspect-video overflow-hidden border-2 border-foreground bg-foreground">
                ...
                <span className="absolute bottom-0 left-0 flex h-11 w-11 items-center justify-center border-r-2 border-t-2 border-foreground bg-background group-hover:bg-teal">
                  <Play
                    className="ml-0.5 h-4 w-4 fill-current text-foreground group-hover:text-white"
                    strokeWidth={2}
                  />
                </span>
              </div>
              <div className="mt-3">
                ...
                {v.result && (
                  <p className="mt-2 text-[13px] font-bold text-teal leading-snug">
                    {v.result}
                  </p>
                )}
              </div>
        ...
        {/* Written quotes along the bottom */}
        <div className="slab mt-6 grid grid-cols-1 gap-[2px] bg-foreground md:grid-cols-3">
          {written.map((w) => (
            <article key={w.name} className="flex flex-col bg-surface p-7 md:p-9">
              {w.result && (
                <p className="mb-5 text-[13px] font-bold uppercase tracking-[0.08em] text-teal">
                  {w.result}
                </p>
              )}
      ...
      {active && (
        ...
        >
          ...
          <div
            className="aspect-video w-full max-w-4xl overflow-hidden border-2 border-white bg-black"
